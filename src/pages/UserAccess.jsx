...
export default function UserAccess() {
  ...
  const [audit, setAudit] = useState(["Users & Access module opened.", "Feature Access now updates the sidebar immediately."]);
  ...
  const updateFeatures = (nextFeatures, log) => {
    ...
    saveFeatureAccess(nextFeatures);
    if (log) setAudit((items) => [log, ...items]);
  };
    ...
    <div className="space-y-6">
      <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
        <div>
          ...
          <p className="mt-2 max-w-3xl text-sm text-slate-400">Add users and control what each role can see. Changes apply to the sidebar immediately.</p>
        </div>
        <button onClick={() => setShowAdd(true)} className="inline-flex items-center justify-center gap-2 rounded-xl bg-green-500 px-4 py-3 text-sm font-bold text-slate-950 shadow-lg shadow-green-500/20 hover:bg-green-400">
          <UserPlus className="h-4 w-4" /> Add User
        </button>
      </div>

      <div className="grid gap-4 md:grid-cols-4">
        {[["Total Users", users.length, Users], ["Active", users.filter((u) => u.account === "Active").length, CheckCircle2], ["Pending Invites", pendingInvites.length, Clock], ["Roles", roles.length, ShieldCheck]].map(([label, value, Icon]) => (
      ...
      <div className="rounded-2xl border border-white/10 bg-white/[0.04] p-2 shadow-xl">
        <div className="flex flex-wrap gap-2">
          {tabs.map(({ id, label, icon: Icon }) => (
            <button key={id} onClick={() => setActiveTab(id)} className={`inline-flex items-center gap-2 rounded-xl px-4 py-2 text-sm font-semibold transition ${activeTab === id ? "bg-green-500 text-slate-950" : "text-slate-300 hover:bg-white/10 hover:text-white"}`}>
        ...
        <div className="rounded-2xl border border-white/10 bg-white/[0.04] shadow-xl">
          <div className="flex flex-col gap-3 border-b border-white/10 p-4 lg:flex-row">
            <div className="relative flex-1">
              ...
              <input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search by name or email..." className="w-full rounded-xl border border-white/10 bg-white/5 py-3 pl-10 pr-4 text-sm text-white outline-none placeholder:text-slate-500 focus:border-green-500/50" />
            </div>
            <select value={roleFilter} onChange={(e) => setRoleFilter(e.target.value)} className="rounded-xl border border-white/10 bg-slate-900 px-4 py-3 text-sm text-white outline-none focus:border-green-500/50">
          ...
          <div className="overflow-x-auto">
            <table className="w-full min-w-[900px] text-left">
              <thead className="bg-white/[0.03] text-xs uppercase tracking-wider text-slate-400"><tr><th className="px-4 py-3">User</th><th className="px-4 py-3">Role</th><th className="px-4 py-3">Account</th><th className="px-4 py-3">Invite Status</th><th className="px-4 py-3">Joined</th><th className="px-4 py-3 text-right">Actions</th></tr></thead>
              <tbody className="divide-y divide-white/10">
                {filteredUsers.map((user) => (
                  <tr key={user.id} className="hover:bg-white/[0.03]">
                    ...
                    <td className="px-4 py-4"><select value={user.role} onChange={(e) => updateUserRole(user.id, e.target.value)} className="rounded-full border border-white/10 bg-slate-900 px-3 py-1 text-xs font-semibold capitalize text-white">{roles.map((role) => <option key={role.value} value={role.value}>{role.label}</option>)}</select></td>
      ...
      {activeTab === "features" && (
        <div className="rounded-2xl border border-white/10 bg-white/[0.04] shadow-xl">
          <div className="border-b border-white/10 p-5"><div className="flex items-center gap-2 text-white"><Settings2 className="h-5 w-5 text-green-400" /><h2 className="text-xl font-bold">Feature Access Matrix</h2></div><p className="mt-2 text-sm text-slate-400">Enable or disable app sections by role. Sidebar changes immediately in the current browser.</p></div>
          <div className="overflow-x-auto">
            <table className="w-full min-w-[850px] text-left">
              <thead className="bg-white/[0.03] text-xs uppercase tracking-wider text-slate-400"><tr><th className="px-4 py-3">Section</th>{roles.map((role) => <th key={role.value} className="px-4 py-3 text-center">{role.label}</th>)}</tr></thead>
              <tbody className="divide-y divide-white/10">
                {featureSections.map((section) => <tr key={section} className="hover:bg-white/[0.03]"><td className="px-4 py-4 font-semibold text-white">{section}</td>{roles.map((role) => <td key={role.value} className="px-4 py-4 text-center"><button onClick={() => toggleFeature(role.value, section)} className={`mx-auto flex h-7 w-12 items-center rounded-full p-1 transition ${featureAccess[role.value]?.[section] ? "bg-green-500" : "bg-slate-700"}`} title={`${featureAccess[role.value]?.[section] ? "Disable" : "Enable"} ${section} for ${role.label}`}><span className={`h-5 w-5 rounded-full bg-white transition ${featureAccess[role.value]?.[section] ? "translate-x-5" : "translate-x-0"}`} /></button></td>)}</tr>)}
              </tbody>
            </table>
          </div>
        </div>
