...
const roleDescriptions = {
  super_admin: {
    label: "Super Admin",
    description: "Platform owner role for API integrations, diagnostics, cleanup tools, security, global toggles, and future multi-company controls.",
  },
  admin: {
    label: "Admin",
    description: "Administration role for finance, fleet, safety, compliance, support, settlements, documents, customers, and users.",
  },
  dispatcher: {
    label: "Dispatcher",
    description: "Operations role for load marketplace, driver matching, bid review, dispatch board, tracking, documents, and messages.",
  },
  driver: {
    label: "Driver",
    description: "Driver app role for assigned loads, load offers, scan, chat, profile, settlements, documents, and emergency workflows.",
  },
  customer: {
    label: "Customer",
    description: "Unified customer portal role for brokers, direct clients, and shippers: shipments, quotes, tracking, documents, invoices, and support.",
  },
};

const roles = rolesForAccess.map((value) => ({ value, ...roleDescriptions[value] }));

const defaultUsers = [
  { id: "usr-owner", name: "Brian M", email: "netzeus20@gmail.com", phone: "", role: "super_admin", account: "Active", inviteStatus: "Accepted", joined: "Jun 24, 2026" },
  { id: "usr-admin", name: "Demo Admin", email: "admin@hasten.com", phone: "", role: "admin", account: "Active", inviteStatus: "Accepted", joined: "Jun 24, 2026" },
...
function loadUsers() {
  return loadState(STORAGE_USERS, defaultUsers).map((user) => {
    if (user.id === "usr-owner" || user.email === "netzeus20@gmail.com") {
      return { ...user, role: user.role === "admin" ? "super_admin" : user.role };
    }
    return user;
  });
}
...
function Toggle({ checked, onClick, disabled = false }) {
  return (
    <button
      disabled={disabled}
      onClick={onClick}
      className={`mx-auto flex h-7 w-12 items-center rounded-full p-1 transition ${checked ? "bg-green-500" : "bg-slate-700"} ${disabled ? "cursor-not-allowed opacity-50" : ""}`}
      type="button"
    >
...
function roleTone(role) {
  ...
  if (role === "dispatcher") return "blue";
  if (role === "driver") return "orange";
  return "slate";
}
...
export default function UserAccess() {
  ...
  const pendingInvites = users.filter((user) => user.inviteStatus !== "Accepted");
  ...
  const updateFeatures = (nextFeatures, log) => {
    const normalized = saveFeatureAccess(nextFeatures);
  ...
  const addUser = (event) => {
    ...
    updateUsers([nextUser, ...users], `Invite prepared for ${nextUser.email} as ${nextUser.role}.`);
    setNewUser({ name: "", email: "", phone: "", role: "driver" });
    setShowAdd(false);
  };

  const updateUserRole = (id, role) => {
    updateUsers(users.map((user) => (user.id === id ? { ...user, role } : user)), `User role changed to ${role}.`);
  };

  const toggleAccount = (id) => {
    updateUsers(
      users.map((user) => (user.id === id ? { ...user, account: user.account === "Active" ? "Disabled" : "Active" } : user)),
  ...
  const removeUser = (id) => {
    ...
    updateUsers(users.filter((item) => item.id !== id), `User removed: ${user?.email || id}.`);
  };

  const toggleFeature = (role, section) => {
    const roleAccess = { ...(featureAccess[role] || defaultFeatureAccess[role] || {}) };
    roleAccess[section] = roleAccess[section] === false;
    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-4 md:grid-cols-4">
        {[
          ...
          ["Active", users.filter((user) => user.account === "Active").length, CheckCircle2],
          ...
          <div key={label} className="rounded-2xl border border-white/10 bg-white/[0.04] p-5 shadow-xl">
            <div className="flex items-center justify-between"><span className="text-sm text-slate-400">{label}</span><Icon className="h-5 w-5 text-green-400" /></div>
      ...
      <div className="rounded-2xl border border-white/10 bg-white/[0.04] p-2 shadow-xl">
        <div className="flex flex-wrap gap-2">
          {tabs.map(({ id, label, icon: Icon }) => (
            <button key={id} onClick={() => setActiveTab(id)} className={`inline-flex items-center gap-2 rounded-xl px-4 py-2 text-sm font-semibold transition ${activeTab === id ? "bg-green-500 text-slate-950" : "text-slate-300 hover:bg-white/10 hover:text-white"}`} type="button">
        ...
        <div className="rounded-2xl border border-white/10 bg-white/[0.04] shadow-xl">
          <div className="flex flex-col gap-3 border-b border-white/10 p-4 lg:flex-row">
            <div className="relative flex-1">
              ...
              <input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Search by name or email..." className="w-full rounded-xl border border-white/10 bg-white/5 py-3 pl-10 pr-4 text-sm text-white outline-none placeholder:text-slate-500 focus:border-green-500/50" />
            </div>
            <select value={roleFilter} onChange={(event) => setRoleFilter(event.target.value)} className="rounded-xl border border-white/10 bg-slate-900 px-4 py-3 text-sm text-white outline-none focus:border-green-500/50">
          ...
          <div className="overflow-x-auto">
            <table className="w-full min-w-[900px] text-left">
              <thead className="bg-white/[0.03] text-xs uppercase tracking-wider text-slate-400">
                <tr><th className="px-4 py-3">User</th><th className="px-4 py-3">Role</th><th className="px-4 py-3">Account</th><th className="px-4 py-3">Invite Status</th><th className="px-4 py-3">Joined</th><th className="px-4 py-3 text-right">Actions</th></tr>
              </thead>
              <tbody className="divide-y divide-white/10">
                {filteredUsers.map((user) => (
                  <tr key={user.id} className="hover:bg-white/[0.03]">
                    <td className="px-4 py-4"><div className="font-semibold text-white">{user.name}</div><div className="text-sm text-slate-400">{user.email}</div></td>
                    <td className="px-4 py-4">
                      <select value={user.role} onChange={(event) => updateUserRole(user.id, event.target.value)} className="rounded-full border border-white/10 bg-slate-900 px-3 py-1 text-xs font-semibold capitalize text-white">
                        {roles.map((role) => <option key={role.value} value={role.value}>{role.label}</option>)}
                      </select>
                    </td>
                    ...
                    <td className="px-4 py-4">
                      <div className="flex justify-end gap-2">
                        <button title="Reset password" className="rounded-lg p-2 text-slate-400 hover:bg-white/10 hover:text-white" type="button"><KeyRound className="h-4 w-4" /></button>
                        <button title="Send invite" className="rounded-lg p-2 text-orange-400 hover:bg-orange-500/10" type="button"><Mail className="h-4 w-4" /></button>
                        <button title="Enable / disable user" onClick={() => toggleAccount(user.id)} className="rounded-lg p-2 text-blue-400 hover:bg-blue-500/10" type="button"><UserX className="h-4 w-4" /></button>
                        <button title="Delete user" onClick={() => removeUser(user.id)} className="rounded-lg p-2 text-red-400 hover:bg-red-500/10" type="button"><Trash2 className="h-4 w-4" /></button>
                      </div>
                    </td>
                  </tr>
                ))}
          ...
          <div className="mt-4 space-y-3">
            {pendingInvites.length === 0 ? <p className="text-sm text-slate-400">No pending invites.</p> : pendingInvites.map((invite) => (
              <div key={invite.id} className="flex flex-col gap-3 rounded-xl border border-white/10 bg-white/[0.03] p-4 sm:flex-row sm:items-center sm:justify-between">
                ...
                <button className="rounded-lg bg-orange-500/10 px-3 py-2 text-sm font-semibold text-orange-300 hover:bg-orange-500/20" type="button">Resend Invite</button>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
          {roles.map((role) => (
            <div key={role.value} className="rounded-2xl border border-white/10 bg-white/[0.04] p-5 shadow-xl">
              <div className="flex items-center justify-between"><h2 className="text-xl font-bold text-white">{role.label}</h2><Pill tone={roleTone(role.value)}>{role.value}</Pill></div>
      ...
      {activeTab === "audit" && (
        <div className="rounded-2xl border border-white/10 bg-white/[0.04] p-5 shadow-xl">
          ...
          <div className="mt-4 space-y-3">
            {audit.map((entry, index) => <div key={`${entry}-${index}`} className="rounded-xl border border-white/10 bg-white/[0.03] p-4 text-sm text-slate-300"><span className="text-slate-500">Now</span> — {entry}</div>)}
          </div>
        </div>
      )}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-4">
          <form onSubmit={addUser} className="w-full max-w-lg rounded-2xl border border-white/10 bg-slate-950 p-6 shadow-2xl">
            <div className="flex items-center justify-between">
              <div><h2 className="text-2xl font-black text-white">Add User</h2><p className="mt-1 text-sm text-slate-400">Create a local demo user or prepare an invitation workflow.</p></div>
            ...
            <div className="mt-6 space-y-4">
              <input value={newUser.name} onChange={(event) => setNewUser({ ...newUser, name: event.target.value })} placeholder="Full Name" className="w-full rounded-xl border border-white/10 bg-white/5 px-4 py-3 text-white outline-none placeholder:text-slate-500 focus:border-green-500/50" />
              <input value={newUser.email} onChange={(event) => setNewUser({ ...newUser, email: event.target.value })} placeholder="Email" type="email" className="w-full rounded-xl border border-white/10 bg-white/5 px-4 py-3 text-white outline-none placeholder:text-slate-500 focus:border-green-500/50" />
              <input value={newUser.phone} onChange={(event) => setNewUser({ ...newUser, phone: event.target.value })} placeholder="Phone (optional)" className="w-full rounded-xl border border-white/10 bg-white/5 px-4 py-3 text-white outline-none placeholder:text-slate-500 focus:border-green-500/50" />
              <select value={newUser.role} onChange={(event) => setNewUser({ ...newUser, role: event.target.value })} className="w-full rounded-xl border border-white/10 bg-slate-900 px-4 py-3 text-white outline-none focus:border-green-500/50">
                {roles.map((role) => <option key={role.value} value={role.value}>{role.label}</option>)}
              </select>
            </div>
  ...
  return (
    <>
      ...
      {group.items.map((section) => (
        <tr key={`${group.key}-${section}`} className="hover:bg-white/[0.03]">
          <td className="px-4 py-3"><div className="pl-5 text-sm font-semibold text-slate-200">↳ {section}</div></td>
          {roles.map((role) => (
            <td key={`${section}-${role.value}`} className="px-4 py-3 text-center">
              <Toggle checked={featureAccess[role.value]?.[section] !== false} disabled={section === "Users & Access"} onClick={() => toggleFeature(role.value, section)} />
            </td>
          ))}
        </tr>
      ))}
