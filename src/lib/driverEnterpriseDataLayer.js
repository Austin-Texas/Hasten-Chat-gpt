...
export function upsertDriverEnterpriseRecord(entityKey, record, actor = "system") {
  ...
  const nextRecord = {
    ...
    id,
    updated_at: nowIso(),
    created_at: existingIndex >= 0 ? existingRows[existingIndex].created_at : nowIso(),
  };
  const rows = existingIndex >= 0
    ? existingRows.map((row, index) => (index === existingIndex ? { ...row, ...nextRecord } : row))
    : [nextRecord, ...existingRows];
  const auditEvent = {
    id: makeId("audit"),
    actor,
  ...
  for (const [entityKey, rows] of Object.entries(recordsByEntity)) {
    ...
    rows.forEach((row) => {
      ...
      existingById.set(id, {
        ...
        id,
        updated_at: nowIso(),
        created_at: existingById.get(id)?.created_at || row.created_at || nowIso(),
      });
    });
...
export function evaluateDriverEnterpriseCompliance(driverId) {
  ...
  function checkRequired(label, record, dateField = "expiration_date") {
    ...
    if (record.verification_status && !["verified", "approved", "valid"].includes(record.verification_status)) {
  ...
  const dqfApproved = docs.some((doc) => doc.doc_type === "DQF" && doc.verified);
  if (!dqfApproved) warnings.push("DQF file incomplete");
