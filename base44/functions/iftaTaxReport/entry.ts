...
function estimateMilesByState(originState, destState, totalMiles) {
  ...
  if (originState === destState || !destState) return { [originState]: totalMiles };
  if (!originState) return { [destState]: totalMiles };
  const half = Math.round(totalMiles / 2);
  return { [originState]: half, [destState]: totalMiles - half };
}
...
Deno.serve(async (req) => {
  try {
    ...
    const allLoads = await base44.asServiceRole.entities.Load.filter({ status: 'completed' }, '-created_date', 500);
